...
class AcademicTasksController extends GetxController {
  Future<void> addTask(Map<String, dynamic> taskData) async {
    try {
      await fireStore.collection(academyTasks).add(taskData);
  ...
  Future<void> updateTask(String docId, Map<String, dynamic> updatedData) async {
    try {
      await fireStore.collection(academyTasks).doc(docId).update(updatedData);
  ...
  Future<void> deleteTask(String docId) async {
    try {
      await fireStore.collection(academyTasks).doc(docId).delete();
  ...
  void showTaskForm(BuildContext context, {String? docId, Map<String, dynamic>? existingData}) {
    if (existingData != null) {
      ...
      codeController.text = existingData['code'] ?? '';
    } else {
            ...
            TextButton(
              onPressed: () {
                final taskData = {
                  ...
                  'button_navigator': existingData?['button_navigator'] ?? [],
